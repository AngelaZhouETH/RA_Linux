...
static double xfov = 0.9; // half-angle in radians
...
static double position_sampling = 0.1;
static double angle_sampling = RN_PI / 9.0;
...
printf("3 Create Room Cameras\n");
  ...
  for (int i = 0; i < scene->NNodes(); i++) {
    ...
    for (int j = 0; j < nangles; j++) {
      ...
      if(zeroscounter >= 8) break; 

      //printf("* # %d\n", i);

      R3Box room_bbox = room_node->BBox();
      float volume = (room_bbox.ZMax() - room_bbox.ZMin()) * (room_bbox.XMax() - room_bbox.XMin()) *(room_bbox.YMax() - room_bbox.YMin());
      //printf("volume %d --- %2f , %2f\n", int(volume), room_bbox.YMin(), room_bbox.YMax());
      float position_samplingZ = (  (room_bbox.ZMax() - room_bbox.ZMin()) - 2*margin)/5;
      float position_samplingX = (  (room_bbox.XMax() - room_bbox.XMin()) - 2*margin)/5;
      R2Point center_position( (room_bbox.XMax() - room_bbox.XMin())/2 + room_bbox.XMin(), (room_bbox.ZMax() - room_bbox.ZMin())/2 + room_bbox.ZMin());
      //printf("center_position: %f , %f \n", center_position.X(), center_position.Y());

	if (volume >=40.0 && room_bbox.YMin()<= 0.5 && room_bbox.YMax()<= 3.5 )
	      for (RNScalar z = room_bbox.ZMin() + margin; z < room_bbox.ZMax() - margin; z += position_samplingZ) {
		for (RNScalar x = room_bbox.XMin() + margin; x < room_bbox.XMax() - margin; x += position_samplingX) {

		  tries++;
		  // Compute position
		  R2Point position(x + position_sampling*(RNRandomScalar()/4), z + position_sampling*(RNRandomScalar()/4));
		  ...
		  towards.Normalize();
 		  if( (   (position.X() - center_position.X())*towards.X() > 0   )  ||  (  (position.Y() - center_position.Y())*towards.Z()  > 0 ) ){
      ...
      if (best_camera.Value() > 0) {
        ...
        cameras.Insert(camera);
        camera_count++;
	printf("Current number of cameras= %d\n", camera_count);
	zeroscounter=0;
      }
      else{
	  zeroscounter++;
