...
dataRootfolder =  '/home/iancher/Experiments/sscnet/demo'; %dataRootfolder = '/n/fs/suncg/voxelLabelComplete/sscnet_release/data';
CAD_3D_gtpath  = '../NYUCAD_3D/';
test = 1; % set it to 0 to save out files for training images
savefiles = 1; % set it to 1 to save out files it might overright existing files
...
for i = 1:length(allSeq)
    ...
    [gridPtsWorldX,gridPtsWorldY,gridPtsWorldZ] = ndgrid(voxOriginWorld(1):voxUnit:(voxOriginWorld(1)+(voxSize(1)-1)*voxUnit), ...
                                                         voxOriginWorld(2):voxUnit:(voxOriginWorld(2)+(voxSize(3)-1)*voxUnit), ...
                                                         voxOriginWorld(3):voxUnit:(voxOriginWorld(3)+(voxSize(2)-1)*voxUnit));
